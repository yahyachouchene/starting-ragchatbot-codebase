...
from ai_generator_pipeline import (
    AIGeneratorPipeline, RoundContext, RoundState, RoundEvent,
    InitialQueryProcessor, SequentialToolProcessor, SynthesisProcessor,
...
class TestPipelineArchitecture:
    ...
    def test_single_round_with_tool_usage(self):
        ...
        with patch.object(self.generator.client.messages, 'create') as mock_create:
            ...
            tool_response.content = [
                Mock(type="tool_use", name="search_course_content", id="tool_1",
            ...
            # Mock tool execution
            self.mock_tool_manager.execute_tool.return_value = "Course content about basic AI concepts"
    ...
    def test_two_round_sequential_tool_usage(self):
        ...
        with patch.object(self.generator.client.messages, 'create') as mock_create:
            ...
            round2_response.content = [
                Mock(type="tool_use", name="search_course_content", id="tool_2",
            ...
            final_response = Mock()
            final_response.stop_reason = "end_turn"
            final_response.content = [Mock(text="Lesson 1 focuses on basics while lesson 2 covers advanced topics...")]
            ...
            assert result == "Lesson 1 focuses on basics while lesson 2 covers advanced topics..."
    ...
    def test_max_rounds_reached_synthesis(self):
        ...
        with patch.object(self.generator.client.messages, 'create') as mock_create:
            ...
            round1_response = Mock()
            round1_response.stop_reason = "tool_use"
            round1_response.content = [Mock(type="tool_use", name="search_course_content", id="1")]
            
            round2_response = Mock()
            round2_response.stop_reason = "tool_use"
            round2_response.content = [Mock(type="tool_use", name="search_course_content", id="2")]
            
            # Synthesis round (no tools available)
            synthesis_response = Mock()
            synthesis_response.stop_reason = "end_turn"
            synthesis_response.content = [Mock(text="Based on the searches, here's a summary...")]
            
            mock_create.side_effect = [round1_response, round2_response, synthesis_response]
    ...
    def test_error_recovery_and_rollback(self):
        ...
        with patch.object(self.generator.client.messages, 'create') as mock_create:
            # First call fails
            mock_create.side_effect = [Exception("API Error"), Mock(content=[Mock(text="Fallback response")])]
...
class TestArchitecturalComparison:
    ...
    def test_event_driven_transitions(self):
        ...
        assert transitions[RoundEvent.TOOL_EXECUTED_CONTINUE] == RoundState.FIRST_TOOL_ROUND
    ...
    def test_error_recovery_granularity(self):
        ...
        assert context.current_state == RoundState.SECOND_TOOL_ROUND  # Stays same if valid rollback
...
class TestEdgeCaseHandling:
    ...
    def test_infinite_loop_prevention(self):
        ...
        result_context = orchestrator.execute_pipeline(context, mock_client, mock_tools, mock_tool_manager)
    ...
    def test_tool_execution_failure_recovery(self):
        ...
        with patch.object(generator.client.messages, 'create') as mock_create:
            ...
            tool_response = Mock()
            tool_response.stop_reason = "tool_use"
            tool_response.content = [Mock(type="tool_use", name="test_tool", id="1", input={})]
...
if __name__ == "__main__":
    ...
    def run_demo_scenarios():
        ...
        print("   Expected: INITIAL_QUERY → FIRST_TOOL_ROUND → SECOND_TOOL_ROUND → COMPLETED")
        ...
        print("   Expected: INITIAL_QUERY → FIRST_TOOL_ROUND → SYNTHESIS_ROUND → COMPLETED")
